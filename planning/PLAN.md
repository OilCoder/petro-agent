...
- [!] Implement multi-seed robustness check: three fixed seed sets; P50 agreement within 1% relative, P10–P90 widths within 5%, consistent tier; failures logged as degradation with `confidence_impact = uncertainty_widening` (`src/evaluation/robustness.py`) (BLOCKED 2026-06-25: `src/evaluation/robustness.py` was never created; the MC uses a single fixed seed (42) — multi-seed robustness not implemented. See auditoría BC-06, DECISIONS D10)
...
- [x] Extend `claim_verifier` with the tone/limitation check (4) — a bracketed/abstaining run must use hedging/limitation language, else flagged (`src/agents/claim_verifier.py`); positional range reconciliation (2)/(3) left as a refinement (2026-06-25)
- [x] Re-tune carbonate cutoffs (sw 0.50, vsh 0.35, phie 0.10 vs corrected PHIE) (`src/params/regional_defaults.json`) (2026-06-25)
- [x] Pin Rw from a water zone via the Rwa method (RT·PHIE^m/a low percentile over clean porous rock) instead of the flat 0.04; data-driven Rw feeds Sw and the uncertainty base (`src/petrophysics/lithology.py`, `src/orchestrator/stages.py`, `graph.py`) (2026-06-25)
...
- [x] Amend the Charter success criterion 4 (statistical calibration) to "infra-ready, unmeasured" and add decisions (b) soft-abstention and (e) ECE-deferred to the MANIFEST (`planning/blueprint/00_charter.md`, `MANIFEST.md`) (2026-06-25)
- [x] Reconcile PLAN tasks citing non-existent files: renames recorded (D10), genuinely-unbuilt items (robustness.py, data_quality.py validator, HCPV) flagged BLOCKED/noted (`planning/PLAN.md`, `DECISIONS.md`) (2026-06-25)
