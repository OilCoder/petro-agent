...
- **Comment language**: English (code and comments). Spanish is reserved for
  `planning/bitacora/` and `aprendizaje/` only.
- **Naming**: `snake_case` (Python). Petrophysical symbols keep their domain names
  (`vsh`, `phie`, `sw`, `rhob`, `nphi`, `gr`, `rt`) — clarity over genericity.
